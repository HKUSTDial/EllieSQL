...
from .prompts.direct_dc_prompt import DIRECT_DC_PROMPT
from ...core.utils import load_json
from .submodules.refiner import *

class DirectDCRefineSQLGenerator(SQLGeneratorBase):
    """Implementation of using direct_dc prompt to generate SQL, with a refiner (direct_dc + refiner)"""
    ...
    def __init__(self, 
                ...
                max_tokens: int = 1000,
                max_retries: int = 3):
        super().__init__("DirectDCRefineSQLGenerator", max_retries)
        self.llm = llm
        self.model = model
    ...
    async def generate_sql(self, query: str, schema_linking_output: Dict, query_id: str, module_name: Optional[str] = None) -> str:
        ...
        messages = [
            ...
            {"role": "user", "content": DIRECT_DC_PROMPT.format(
