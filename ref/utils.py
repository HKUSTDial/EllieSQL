...
from sql_execution import execute_sql_without_timeout
...
def load_database_description(db_id: str, database_root_dir: str) -> Dict[str, Dict[str, Dict[str, Any]]]:
    ...
    for csv_file in db_description_dir.glob("*.csv"):
        ...
        for _, row in table_description_df.iterrows():
            ...
            column_description = row["column_description"].replace("\r", "").replace("\n", "").replace("commonsense evidence:", "").strip() if pd.notna(row["column_description"]) else ""
            data_format = row["data_format"].strip() if pd.notna(row["data_format"]) else ""
            value_description = row["value_description"].replace("\r", "").replace("\n", "").replace("commonsense evidence:", "").strip() if pd.notna(row["value_description"]) else ""
...
def build_table_ddl_statement(table_schema_dict: Dict[str, Dict[str, Any]],
    ...
    for column_name, column_schema in table_schema_dict["columns"].items():
        ...
        comment_parts = []
        if add_expanded_column_name and column_schema["expanded_column_name"].strip() != "":
            comment_parts.append(f"Column Meaning: {column_schema['expanded_column_name']}")
        if add_column_description and column_schema["column_description"].strip() != "":
        ...
        if add_value_examples and len(column_schema["value_examples"]) > 0:
